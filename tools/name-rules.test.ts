...
/**
 * 날짜 규칙은 **날짜 꼴만** 버린다(2026-09-26). 처음 판은 이름 안의 「숫자+년/월/일」을 무조건 버려
 * 고유명·제도명까지 잃었다 — 캐시에서 버림 12건 중 10건이 그랬다.
 */
describe("날짜 꼴과 이름 속 숫자", () => {
  it.each([
    ["10월 혁명", 1917],
    ["1월 폭풍", 1967],
    ["38년 전쟁 발발", 774],
    ...
    expect(validName(name, "…", year)).toBe(name);
  });
  ...
  it.each([
    ...
    ["6월 28일 개통", 2007],
    ["12월 7일 기습", 1941],
  ])("「%s」는 날짜가 섞여 버린다", (name, year) => {
    expect(validName(name, "…", year)).toBeNull();
  });
});
