 ...
 * 세기 표기(「BC.3세기경 」)와 숫자 뒤 기원 표기(「300 BC: 」)도 뗀다 — 전에는 「세기 한반도 일대에…」「BC: Beginning of
 * the Iron Age」가 제목으로 남아 칩에 「기원전: 철기 시대 시작」이 떴다(2026-09-26, 13줄).
 * 연도 뒤 불확실 표시 「?」도 뗀다 — 「180年頃? 倭国大乱が勃発」이 칩에 「? 왜국대란이 발발」로 떴다(2026-09-26).
 */
export const stripYear = (t) =>
  t
    .replace(
      // 원호 괄호는 한 겹 중첩까지 — "1543年（天文12年、一説に1542年（天文11年））"
      /^\s*(?:約|约|およそ)?\s*(?:BC\.?\s*|기원전\s*|紀元前|公元前|西元前|前)?\d+\s*(?:만|万|萬)?\s*(?:年代?|년|세기)?\s*(?:(?:BCE?|AD|CE)(?![A-Za-z]))?\s*(?:경|말|초|쯤|중반|후반|전반|頃|ごろ|左右|前後)?\s*[?？]?\s*(?:（(?:[^（）]|（[^（）]*）)*）)?\s*[:：\-–—]?\s*/,
      "",
    )
    .replace(/^\d{1,4}:\s*/, "")
    .trim();
