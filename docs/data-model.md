...
## 5. 처리량 — 2026-09-04 프로브 실측

`tools/source-probe.mjs`로 §4-1 원천을 전부 받아 **후보 행**(첫 셀이 연도로 읽히는 표 행 + 연도로 시작하는 목록 항목)을 셌다. 전체 보고서는 `curation/_probe/report.md`.

| 열 | 후보 행 | 0.4 목표 | 판정 |
|---|---|---|---|
| 한국 | 907 | 1,000 | 근접하나 **신뢰도 낮음** — ko:한국사 연표의 링크 보유가 38%라 오탐이 많다. en 연표(166항목·링크 99%)가 실질 원천일 수 있다 |
| 중국 | 1,082 | 800 | **충분.** 링크 98% |
| 일본 | 1,581 | 600 | 수량은 넉넉하나 링크 35% — 한국과 같은 오탐 문제 |
| 미국 | 279 | 600 | **부족.** 목표의 절반 미만. 링크율이 89~100%로 높아 이 수치는 신뢰할 만하다(PRD §11 C-12) |
...
- **MVP 목표 3,000건은 미국 열 때문에 그대로 유지할 수 없다.** 목표 재설정은 C-12 결정 후.
