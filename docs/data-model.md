...
|---|---|
| 버전 | 0.3 (초안) |
| 작성일 | 2026-09-03 (0.3 갱신 2026-09-04) |
| 변경 이력 | 0.1 → 0.2: `event_names`(관점별 명칭) 신설. 기본 4열을 한국·중국·일본·미국으로 변경하고 유럽 롤업은 P1로 이동. `regions`에 수록 범위 필드 추가(PRD §11 C-3). 청크 발행을 연속 줌 전제로 정리(§6)<br>0.2 → 0.3: 기원전 세기 라벨 오류 수정(§3-4). **청크 파일 단위 확대**(연도 1년 → 10년 묶음 등)와 **요약문 분리**를 네이티브 스크롤 전환에 맞춰 반영(§6). PRD 0.4 §5-5·§5-5A 연동. 대표 지시 2026-09-04 |
| 상태 | 스키마 확정 전. 마이그레이션은 이 문서 확정 후 작성 |
| 관련 문서 | `docs/PRD.md` §5-3, §5-5A(시간축 좌표계), §6 · `docs/editorial-policy.md` |
| 저장소 | Supabase(Postgres) — 어드민·큐레이션 전용. 읽기 경로는 §6 정적 JSON |
...
| 2 | en.wikipedia Timeline of United States history | **미국(P0, 기본 4열)** | CC BY-SA 4.0 | 시기별 하위 문서 순회 필요. 1607년부터 수록 |
| 3 | Wikidata | 전체 | CC0 | QID·날짜 정밀도·사이트링크. 동아시아 전근대 커버리지 약함 |
| 4 | ko.wikipedia 「일본사 시대 구분표」, 각 왕조 문서 | polities | CC BY-SA 4.0 | 시대 밴드 시작·종료 연도 |
...
[수집]   연표 문서 파싱 → staging(raw_events: 원문 행, 출처 URL·리비전)
   ↓
[정규화] Wikidata 사이트링크로 QID 부착 → QID 기준 중복 병합
...
모든 전이는 `review_log`에 기록. 어드민은 `/admin/*` Supabase SSR 미들웨어 인증, `/api/admin/*`는 라우트 내부 검증.

---

## 5. 처리량 가정
- 도구가 갖춰지면 큐레이터 1인 시간당 약 100건 **(가정)**. MVP 목표 발행 3,000건(한국 1,000 / 중국 800 / 일본 600 / 미국 600) **(가정)** → 검토 후보 약 5,000건 ≈ 50시간.
- 미국 열은 수록 범위가 1607년부터라 연수는 짧지만 근현대 밀도가 높아 총량은 비슷하다 **(가정)**.
