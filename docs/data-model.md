...
[초안]   LLM(별칭 standard): title_ko(중립 라벨)·summary_ko 초안, category, scope,
         ...
         ※ 한 줄에 뭉친 여러 사건을 쪼갠다(§5-1). 기간 서술은 kind=period로 분리
         ※ **원문 재사용 자동 검사** — summary_ko와 원문의 연속 공통 어절이
            임계값을 넘으면 status=rejected로 떨어뜨린다(editorial-policy §1-6,
            src/lib/curation/overlap.mjs). 검토 큐에 올라가지도 않는다
         구현: tools/draft.mjs. 모델은 src/lib/curation/model.mjs의 DRAFT_MODEL 한 곳에서 정한다
   ↓
...
| * → rejected | 큐레이터 **또는 파이프라인** | 사유 필수. 파이프라인이 거는 유일한 반려는 **원문 재사용 검사 실패**다(editorial-policy §1-6) |
...
- **초안 스키마 누락.** 전승 연대(`historicity`), 근사 연도(`approximate`), 검토 메모(`note`)가 필요했다. `tools/draft.mjs`의 스키마·프롬프트에 추가했다.
